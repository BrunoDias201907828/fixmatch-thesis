...
def train_on_fold(train_subset, val_subset):
    if args.method.lower() == "supervised":
        # All available samples are treated as labelled.
        train_sup_dataset = train_subset
        train_unsup_dataset = None  # sentinel for later conditionals
    else:
        targets = np.array(train_subset.dataset.targets)[train_subset.indices]
        sup_idx, unsup_idx = train_test_split(
            np.arange(len(train_subset)),
            train_size=args.num_labeled,
            stratify=targets,
            random_state=SEED,
        )
        train_sup_dataset = torch.utils.data.Subset(train_subset, sup_idx)
        train_unsup_dataset = torch.utils.data.Subset(train_subset, unsup_idx)
    ...
    if train_unsup_dataset is not None:
        train_unsup_dataloader = torch.utils.data.DataLoader(
            train_unsup_dataset, args.unsup_batchsize, shuffle=True,
            num_workers=4, pin_memory=True
        )
    else:
        # keep the generic loop happy without touching len()
        train_unsup_dataloader = None
    ...
    for epoch in range(args.epochs):
        ...
        if train_unsup_dataloader is None:            # pure-sup
            semisup_n_iter = len(train_sup_dataloader)
            sup_dataloader_iter  = iter(train_sup_dataloader)
            # we just cycle a dummy, so zip() keeps yielding
            unsup_dataloader_iter = cycle([None])
        else:                                     # FixMatch etc.
            semisup_n_iter = max(len(train_sup_dataloader), len(train_unsup_dataloader))
            sup_dataloader_iter  = (cycle(train_sup_dataloader)
                        if len(train_sup_dataloader)  < len(train_unsup_dataloader)
                        else iter(train_sup_dataloader))
            unsup_dataloader_iter = (cycle(train_unsup_dataloader)
                        if len(train_unsup_dataloader) < len(train_sup_dataloader)
                        else iter(train_unsup_dataloader))


        for (sup_imgs, sup_labels), unsup_imgs in zip(sup_dataloader_iter, unsup_dataloader_iter):
